...
function M.buffer_not_empty()
  ...
  if #name == 0 then
    ...
    if info[1].loclist == 0 and info[1].quickfix == 0 then return false end
  end
  return true
end
...
function M.is_read_only()
  if bo.readonly == true or bo.modifiable == false then return true end
  return false
end

function M.is_buffer_changed()
  local info = fn.getbufinfo('')
  if info[1].changed == 1 then return true end
  return false
end

function M.is_git_repository() return b.gitsigns_head and true or false end
